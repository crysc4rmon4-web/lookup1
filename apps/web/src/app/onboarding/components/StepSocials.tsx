...
import { SOCIAL_PLATFORMS, type Platform } from "@lookup/config";
...
export function StepSocials({ links, onChange }: Props) {
  const [showAll, setShowAll] = useState(false);

  const selected = useMemo(
    () => new Set(links.map((item) => item.platform)),
    [links],
  );

  const platforms = showAll
    ? SOCIAL_PLATFORMS.filter((item) => item.enabled)
    : SOCIAL_PLATFORMS.filter((item) => item.enabled && item.featured);

  function toggle(platform: Platform) {
    if (selected.has(platform.id)) {
      onChange(links.filter((item) => item.platform !== platform.id));
  ...
  function update(platformId: string, value: string) {
    onChange(
      links.map((item) =>
  ...
  function remove(platformId: string) {
    onChange(links.filter((item) => item.platform !== platformId));
  }

  function normalizedPreview(value: string) {
    return value.trim();
  }
  ...
  return (
    <section className="flex flex-col">
      ...
      <div className="mt-6 flex justify-center">
        ...
        >
          {showAll ? "Ver menos" : "Ver más redes"}
        </button>
      </div>
        ...
        {links.map((link) => {
          ...
          return (
            ...
            >
              ...
              <input
                value={link.url}
                onChange={(event) => update(platform.id, event.target.value)}
              ...
              {platform.prefix && (
                <p className="mt-3 break-all text-xs text-slate-400">
                  {platform.prefix}
                  {normalizedPreview(link.url) || platform.placeholder}
                </p>
              )}
            </div>
          );
