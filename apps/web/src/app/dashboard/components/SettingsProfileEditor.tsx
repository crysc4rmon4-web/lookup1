...
import type { SettingsEditSection } from "./SettingsView";
...
type SettingsProfileEditorProps = {
  ...
  saving: boolean;
  onSave: (data: SettingsProfileEditorData) => void | Promise<void>;
  onClose: () => void;
};
...
function getTitle(section: SettingsEditSection) {
  switch (section) {
    case "name":
...
function getDescription(section: SettingsEditSection) {
  switch (section) {
    case "name":
...
export function SettingsProfileEditor({
  ...
  const [socialLinks, setSocialLinks] = useState<ProfileLink[]>(links);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if ((section === "name" || section === "profile") && cleanName.length < 2) {
    ...
    if (
      (section === "socials" || section === "profile") &&
      socialLinks.some((link) => link.platform.trim() && !link.url.trim())
    ) {
      setError(
    ...
    const cleanSocialLinks = socialLinks.filter(
      (link) => link.platform.trim() && link.url.trim(),
    );

    await onSave({
      fullName:
        section === "name" || section === "profile"
          ? cleanName
          : (profile.full_name ?? ""),
      profession:
        section === "profession" || section === "profile"
          ? cleanProfession
          : (profile.profession ?? ""),
      bio:
        section === "bio" || section === "profile"
          ? cleanBio
          : (profile.bio ?? ""),
      socialLinks:
        section === "socials" || section === "profile"
  ...
  const showAvatar = section === "profile";

  const showName = section === "name" || section === "profile";

  const showProfession = section === "profession" || section === "profile";

  const showBio = section === "bio" || section === "profile";

  const showSocials = section === "socials" || section === "profile";
            ...
            {showName && (
              <div>
                ...
                <p className="mt-2 text-xs text-slate-400">
                  Tu usuario se mantendrá sincronizado con el nombre cuando sea
                  necesario.
                </p>
              </div>
            )}
