...
export function RadarCanvas({ enabled, profiles }: Props) {
  const profilePositions = useMemo<RadarProfilePosition[]>(() => {
    ...
    return profiles.map((profile) => {
      ...
      const normalizedDistance = distance / MAX_RADIUS_METERS;

      /*
       * Dejamos margen interno para que los puntos
       * nunca puedan tocar el borde del radar.
       */
      const radialPercent = 8 + normalizedDistance * 34;

      const angle = getStableAngle(profile.id);

      const radians = (angle * Math.PI) / 180;

      const left = 50 + Math.cos(radians) * radialPercent;

      const top = 50 + Math.sin(radians) * radialPercent;

      const intensity = Math.max(0, 1 - normalizedDistance);

      const size = 8 + intensity * 5;
    ...
    >
      ...
      {enabled && (
        <div
          ...
          "
          style={{
            animation: "lookup-radar-sweep 3s linear infinite",
            background:
      ...
      {enabled && (
        <div
          ...
          "
          style={{
            animation: "lookup-radar-pulse 2.4s ease-in-out infinite",
          }}
        />
      )}
        ...
        >
          ...
          <span
            ...
            className="
              ...
              "
            style={{
              width: position.size + 22,
              height: position.size + 22,
              transform: "translate(-50%, -50%)",
              animation: "lookup-radar-dot 1.8s ease-in-out infinite",
              opacity: 0.25 + position.intensity * 0.5,
            }}
          />
          ...
          <span
            ...
            style={{
              ...
              boxShadow: `
                  0 0 ${8 + position.intensity * 12}px rgba(255,77,90,0.48),
                  0 0 ${18 + position.intensity * 18}px rgba(255,77,90,0.18)
                `,
            }}
          />
        </div>
